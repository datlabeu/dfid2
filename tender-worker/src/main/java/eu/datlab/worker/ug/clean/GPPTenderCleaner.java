...
public class GPPTenderCleaner extends BaseGPPTenderCleaner {
    ...
    private static final List<CpvCodeAndRule> CpvRules = Arrays.<CpvCodeAndRule>asList(
            ...
            new CpvCodeAndRule("301920001", Arrays.asList("office & supply", "stamp")),
            ...
            new CpvCodeAndRule("391300002", Arrays.asList("office furniture", "table", "tables")),
            ...
            new CpvCodeAndRule("302000001", Arrays.asList("digital", "comnew CpvRuleer", "media",
                    "laptop", "computer", "computers", "equipment & it[SPACE]")),
            ...
            new CpvCodeAndRule("34000000", Arrays.asList("bus")),
            new CpvCodeAndRule("729000009", Arrays.asList("remote backup")),
            new CpvCodeAndRule("30000001", Arrays.asList("agricultural")),
            new CpvCodeAndRule("38600000", Arrays.asList("projector")),
    ...
    /**
     * Create regex for matching title and cpv code rule.
     * @param rule rule
     * @return regex to match rule
     */
    private String createRegexForCPV(final String rule) {
        String cleanedRule = rule.replace("[SPACE]", " ").toLowerCase();
        return ".*"
                + (cleanedRule.startsWith(" ") ? "" : "\\b") + cleanedRule
                + (cleanedRule.endsWith(" ") ? "" : "\\b") + ".*";
    }
    ...
    private boolean cpvFitRule(final String title, final String rule) {
        ...
        if(!rule.contains("&")) {
            return titleLower.matches(createRegexForCPV(rule));
        }
        String[] parts = rule.split(" & ");
        for(String part: parts) {
            if(!titleLower.matches(createRegexForCPV(part))) {
                return false;
            }
        }
        return true;
