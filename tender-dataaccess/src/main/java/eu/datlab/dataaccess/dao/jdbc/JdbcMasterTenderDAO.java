...
public class JdbcMasterTenderDAO extends GenericJdbcDAO<MasterTender> implements MasterTenderApiDAO {
    ...
    @Override
    public final List<MasterTender> getByBuyerAssignedId(final String buyerAssignedId) {
        try {
            String query = "SELECT * FROM " + getTableWithSchema() + " WHERE " +
                    "data @> '{\"buyerAssignedId\": \"" + buyerAssignedId + "\"}'";
    ...
    @Override
    public final List<MasterTender> getBySourceId(final String sourceId) {
        try {
            String query = "SELECT * FROM " + getTableWithSchema() + " WHERE " +
                    "data @> '{\"publications\": [{\"sourceId\": \"" + sourceId + "\"}]}'";
